...
export async function importCustomersToFirestore() {
  ...
  for (const customer of supabaseCustomers) {
    ...
    try {
      ...
      if (error.code === "auth/email-already-in-use") {
        console.log(
          `⚠️ Auth user exists, trying to link Firestore: ${validEmail}`
        );
        ...
        try {
          ...
          console.log(
            `✅ Got UID for existing user: ${validEmail} (UID: ${authUid})`
          );
        } catch (signInError: any) {
          console.log(
            `❌ Could not sign in existing user ${validEmail}: ${signInError.message}`
          );
          skippedCount++;
          continue;
        }
        ...
        console.log(
          `❌ Auth creation failed for ${validEmail}: ${error.message}`
        );
        skippedCount++;
        continue;
      }
    ...
    try {
      ...
      if (!snap.empty) {
        console.log(
          `⚠️ Firestore doc already exists for mobile ${mobile}, skipping`
        );
        skippedCount++;
        continue;
      }
      ...
      console.log(
        `❌ Firestore check failed for ${validEmail}: ${error.message}`
      );
      skippedCount++;
      continue;
    }
  ...
  console.log(
    `🎉 Import complete. Created/Linked: ${createdCount}, Skipped: ${skippedCount}`
  );
}
