...
export default function TasksView({ tasks, setTasks, savedReports, currentUser, teamMembers }: TasksViewProps) {
  ...
  const kpiData = useMemo(() => {
    ...
    const monthlyTarget = 20 * (memberFilter === 'all' && currentUser.role !== 'TECHNICIAN' ? accessibleMembers.length : 1); // Scale target if viewing team
    const targetProgress = Math.min(Math.round((reportsThisMonth / monthlyTarget) * 100), 100);
    ...
    <div className="space-y-8 pb-20 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 flex items-center gap-2">
             My Works <span className="text-xs font-bold text-indigo-600 bg-indigo-50 px-2 py-1 rounded-lg border border-indigo-100">{currentUser.role === 'TECHNICIAN' ? 'Personal View' : 'Team Overview'}</span>
          </h1>
          <p className="text-slate-500 text-sm">Manage tasks and track performance goals.</p>
        </div>
        ...
        <div className="flex items-center gap-3">
           ...
           {accessibleMembers.length > 1 && (
              <div className="relative group">
                 <div className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none"><UserIcon size={14} /></div>
                 <select 
                    value={memberFilter}
                 ...
                 >
                    ...
                    {accessibleMembers.map(m => (
                        <option key={m.id} value={m.id}>{m.id === currentUser.id ? 'My Tasks' : m.name}</option>
                    ))}
                 </select>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-gradient-to-br from-indigo-600 to-blue-600 rounded-2xl p-6 text-white shadow-lg shadow-indigo-200 relative overflow-hidden group">
           ...
           <p className="text-indigo-100 text-xs font-bold uppercase tracking-wider mb-2">Monthly Reports</p>
           ...
           <div className="mt-2 text-[10px] font-bold text-right">{kpiData.targetProgress}% Achieved</div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm flex flex-col justify-between group hover:border-indigo-300 transition-colors">
           <div className="flex justify-between items-start">
              <div>
                 <p className="text-slate-400 text-xs font-bold uppercase tracking-wider mb-1">Active Tasks</p>
              ...
              {kpiData.urgentPending > 0 ? (
                 <div className="bg-red-50 text-red-600 px-2 py-1 rounded-lg text-xs font-bold flex items-center gap-1 animate-pulse">
                    <Flame size={12} fill="currentColor" /> {kpiData.urgentPending} Urgent
                 </div>
              ) : (
        ...
        <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm flex flex-col justify-between group hover:border-indigo-300 transition-colors">
           <div className="flex justify-between items-start">
              <div>
                 <p className="text-slate-400 text-xs font-bold uppercase tracking-wider mb-1">Lifetime Reports</p>
        ...
        <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm flex items-center justify-between group hover:border-indigo-300 transition-colors">
           <div>
              <p className="text-slate-400 text-xs font-bold uppercase tracking-wider mb-1">Task Completion</p>
         ...
         <div className="flex justify-between items-center mb-6">
            ...
            <div className="flex bg-slate-100 p-1 rounded-lg">
               <button 
                 ...
                 className={`px-3 py-1 text-xs font-bold rounded-md transition-all ${chartView === 'monthly' ? 'bg-white shadow text-indigo-600' : 'text-slate-500 hover:text-slate-700'}`}
               >
                 Monthly
               </button>
               <button 
                 onClick={() => setChartView('yearly')}
                 className={`px-3 py-1 text-xs font-bold rounded-md transition-all ${chartView === 'yearly' ? 'bg-white shadow text-indigo-600' : 'text-slate-500 hover:text-slate-700'}`}
               >
                 Yearly
               </button>
            </div>
                     ...
                     >
                        <div className="flex items-center gap-3 overflow-hidden flex-1">
                           ...
                           <div className="flex flex-col overflow-hidden">
                              <span className={`text-sm font-medium truncate ${task.status === 'completed' ? 'line-through text-slate-400' : 'text-slate-700'}`}>
                                 {task.title}
                              </span>
                              <div className="flex items-center gap-2 text-[10px] text-slate-400">
                                 ...
                                 {isAssignedToOther && (
                                    <div className="flex items-center gap-1 bg-slate-100 px-1.5 py-0.5 rounded-md border border-slate-200">
                                       {assignee.photo ? (
                                          <img src={assignee.photo} className="w-3 h-3 rounded-full" alt="" />
                                       ) : (
                                          <div className="w-3 h-3 rounded-full bg-slate-300 flex items-center justify-center text-[8px] text-white font-bold">{assignee.name.charAt(0)}</div>
                                       )}
                        ...
                        {currentUser.role !== 'TECHNICIAN' && task.createdById === currentUser.id && (
  <button 
    onClick={() => deleteTask(task.id)}
    className="p-2 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-lg opacity-0 group-hover:opacity-100 transition-all"
  >
    <Trash2 size={16} />
               ...
               <div className="flex flex-col items-center justify-center h-48 text-slate-400">
                  <div className="w-16 h-16 bg-slate-50 rounded-full flex items-center justify-center mb-4">
                     {statusFilter === 'completed' ? <CheckCircle2 size={32} /> : <Filter size={32} />}
                  </div>
                  <p className="text-sm font-medium">No tasks found</p>
                  <p className="text-xs mt-1 text-slate-400/80">Try changing your filters or add a new task.</p>
               </div>
            )}
         </div>
