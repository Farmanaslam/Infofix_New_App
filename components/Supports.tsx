...
export default function Supports({ tickets, customers, tasks, settings, onUpdateSettings }: SupportsProps) {
  ...
  const insights = useMemo(() => {
    ...
    const keywords = ['screen', 'display', 'battery', 'charging', 'water', 'dead', 'software', 'keyboard', 'hinge', 'heat'];
  ...
  const systemContext = useMemo(() => {
    ...
    const scheduleStatus = todaysTasks.length > 3 ? "HEAVY WORKLOAD" : "LIGHT WORKLOAD";
  ...
  useEffect(() => {
    if (activeTicket) {
      ...
      const custHistory = tickets.filter(t => t.customerId === activeTicket.customerId && t.id !== activeTicket.id);
      
      // Find similar tickets (same device, similar issue keywords)
      const issueWords = activeTicket.issueDescription.split(' ').filter(w => w.length > 3).slice(0, 3);
      const similar = tickets.filter(t => 
          t.status === 'Resolved' && 
          t.deviceType === activeTicket.deviceType && 
          t.id !== activeTicket.id &&
          issueWords.some(w => t.issueDescription.toLowerCase().includes(w.toLowerCase()))
      ).slice(0, 3);
      
      const similarMsg = similar.length > 0 
  ...
  const handleSendMessage = async (text: string = inputValue) => {
    ...
    const kbContext = settings.supportGuidelines.map(g => 
        `[PROTOCOL: ${g.title}]\nCategory: ${g.category}\n${g.content}`
    ).join('\n\n');
    ...
    if (activeTicket) {
        specificContext = `
            ...
            Device: ${activeTicket.brand} ${activeTicket.model} (${activeTicket.deviceType})
            Issue: ${activeTicket.issueDescription}
        ...
        if (activeTicket.history) {
            specificContext += `\nTICKET HISTORY LOGS:\n${activeTicket.history.map(h => `- ${h.date}: ${h.action} (${h.details})`).join('\n')}`;
        }
    } else {
        specificContext = "NO ACTIVE TICKET SELECTED. User is asking general questions.";
    }
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        id: Date.now().toString(),
        role: 'assistant',
        content: "I'm having trouble connecting to the AI service right now. Please check your internet connection.",
        timestamp: new Date()
      }]);
    } finally {
      setIsTyping(false);
  ...
  const handleFormalizePattern = async (pattern: DiscoveredPattern) => {
      ...
      const prompt = `
        Based on these ${pattern.count} resolved tickets about "${pattern.title}", draft a standard service protocol.
        
        Ticket Examples:
        ${pattern.relatedTickets.slice(0, 5).map(t => `- ${t.issueDescription} (Status: ${t.status})`).join('\n')}
      ...
      const aiDraft = await generateAIResponse(prompt, "Drafting a Standard Operating Procedure (SOP).");
      ...
      setTimeout(() => {
          ...
          const inputCat = document.getElementById('g-category') as HTMLInputElement;
  ...
  const handleAddToKnowledgeBase = (content: string) => {
      ...
      setTimeout(() => {
          ...
          if (inputTitle) inputTitle.value = `Learned Solution: ${new Date().toLocaleDateString()}`;
  ...
  const handleSaveGuideline = (g: SupportGuideline) => {
    ...
    if (settings.supportGuidelines.find(item => item.id === g.id)) {
        newGuidelines = settings.supportGuidelines.map(item => item.id === g.id ? g : item);
    } else {
  ...
  const quickActions = activeTicket ? [
    { label: "Diagnose", icon: Stethoscope, prompt: `Diagnose the issue: "${activeTicket.issueDescription}" for ${activeTicket.brand} ${activeTicket.model}. Check if we have similar past tickets.` },
    { label: "Draft Update", icon: Mail, prompt: `Draft a short, polite WhatsApp message for ${activeTicket.name} about their ${activeTicket.deviceType}. Status: ${activeTicket.status}.` },
    { label: "Check Warranty", icon: FileText, prompt: "Does this repair look like a warranty case? What should I verify?" },
    { label: "Parts List", icon: ListChecks, prompt: `List potential spare parts needed for "${activeTicket.issueDescription}".` },
  ] : [
    { label: "Shop Summary", icon: Clock, prompt: "Give me a summary of the shop's current workload, including open tickets and pending tasks." },
    { label: "Identify Trends", icon: TrendingUp, prompt: "Analyze the resolved tickets and tell me what the top 3 most common repair issues are this month." },
    { label: "Search KB", icon: Search, prompt: "Search the knowledge base for 'Screen Replacement' protocols." }
  ];
      ...
      <div className="w-80 flex-shrink-0 flex flex-col bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-100 bg-slate-50/50">
           <h3 className="font-bold text-slate-800 flex items-center gap-2 mb-3">
             <MessageSquare size={18} className="text-indigo-600"/> Ticket Context
           </h3>
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-2 custom-scrollbar bg-slate-50/30">
            ...
            {filteredTickets.map(ticket => (
                ...
                >
                    {selectedTicketId === ticket.id && <div className="absolute left-0 top-0 bottom-0 w-1 bg-indigo-500"></div>}
                    <div className="flex justify-between items-start mb-1">
                        <span className={`text-xs font-bold ${selectedTicketId === ticket.id ? 'text-indigo-600' : 'text-slate-500'}`}>{ticket.ticketId}</span>
                        <span className={`text-[10px] px-1.5 py-0.5 rounded font-bold uppercase ${
                            ticket.priority === 'High' ? 'bg-red-50 text-red-600' : 'bg-slate-100 text-slate-500'
                        }`}>{ticket.priority}</span>
                    </div>
                    <div className="font-semibold text-slate-800 text-sm mb-0.5 truncate flex items-center gap-2">
                       {ticket.deviceType === 'Laptop' ? <Laptop size={12} className="text-slate-400"/> : <Smartphone size={12} className="text-slate-400"/>}
                       {ticket.brand} {ticket.model}
                    </div>
                    <div className="text-xs text-slate-500 line-clamp-2">{ticket.issueDescription}</div>
                </button>
            ))}
            {filteredTickets.length === 0 && (
                <div className="p-8 text-center text-slate-400 text-xs italic">
                   <div className="bg-slate-100 w-10 h-10 rounded-full flex items-center justify-center mx-auto mb-2"><Search size={16}/></div>
            ...
            <div className="flex items-center gap-3">
                ...
                <div>
                    <h2 className="font-bold text-slate-800 text-sm flex items-center gap-2">
                       INFOFIX AI <span className="text-[10px] font-bold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-full border border-indigo-100">PRO</span>
                    </h2>
                    <p className="text-xs text-slate-500 flex items-center gap-1">
                       {activeTicket ? `Active: ${activeTicket.ticketId}` : 'Global Operations Mode'}
                ...
                <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                    <div className={`flex flex-col gap-1 max-w-[85%] ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                        ...
                        {msg.role === 'assistant' && msg.canBeSaved && (
                            ...
                            >
                                <Sparkles size={12} className="text-amber-500 group-hover:scale-110 transition-transform" /> Save this solution
                            </button>
                        )}
                    </div>
                </div>
         ...
         <div className="p-4 bg-white border-t border-slate-200">
             ...
             <div className="relative flex items-center gap-2">
                 <input
                    ...
                    placeholder={activeTicket ? "Ask about current ticket, history, or protocols..." : "Ask about business trends, schedule, or search info..."}
          ...
          <div className="w-96 flex-shrink-0 flex flex-col bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden animate-in slide-in-from-right-4 duration-300">
             <div className="p-4 border-b border-slate-100 bg-slate-50/50">
                ...
                <div className="flex bg-slate-200/50 p-1 rounded-xl mb-3">
                    <button 
                       ...
                       className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-all ${kbTab === 'insights' ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                       Insights {insights.length > 0 && `(${insights.length})`}
                    </button>
                    <button 
                       onClick={() => setKbTab('protocols')}
                       className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-all ${kbTab === 'protocols' ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                       Protocols
                    </button>
                </div>
             ...
             <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
                ...
                {kbTab === 'insights' && (
                    <div className="space-y-3">
                        <div className="p-3 bg-indigo-50 rounded-xl border border-indigo-100 text-[10px] text-indigo-800 leading-relaxed font-medium">
                            <Lightbulb size={14} className="inline mr-1 mb-0.5 fill-current"/>
                            I analyze resolved tickets to find recurring issues. Click on a pattern to generate a standardized protocol.
                        </div>
                        
                        {insights.map(pattern => (
                            <div key={pattern.id} className="p-4 bg-white border border-slate-200 rounded-xl hover:shadow-md transition-all group">
                                <div className="flex justify-between items-start mb-2">
                                    <h4 className="font-bold text-slate-700 text-sm">{pattern.title}</h4>
                                    <span className="bg-emerald-100 text-emerald-700 text-[10px] font-bold px-2 py-0.5 rounded-full">
                                        {pattern.count} Cases
                                    </span>
                                </div>
                                <p className="text-xs text-slate-500 mb-3">{pattern.description}</p>
                                <button 
                        ...
                        {insights.length === 0 && (
                            <div className="text-center py-10 text-slate-400 text-xs">
                                No significant patterns detected yet. Resolve more tickets to see insights.
                            </div>
                        )}
                    </div>
                )}
                ...
                {kbTab === 'protocols' && (
                    <>
                        {filteredKB.map(g => (
                            <div key={g.id} className="p-3 bg-white border border-slate-200 rounded-xl hover:border-indigo-300 hover:shadow-sm transition-all group cursor-default">
                                <div className="flex justify-between items-start mb-2">
                                    <span className="text-[9px] font-bold uppercase tracking-wide text-slate-500 bg-slate-100 px-2 py-0.5 rounded">{g.category}</span>
                                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button onClick={() => { setEditingGuideline(g); setIsGuidelineModalOpen(true); }} className="text-slate-400 hover:text-indigo-600"><Edit2 size={12}/></button>
                                        <button onClick={() => handleDeleteGuideline(g.id)} className="text-slate-400 hover:text-red-600"><Trash2 size={12}/></button>
                                    </div>
                                </div>
                                <h4 className="font-bold text-slate-700 text-xs mb-1 line-clamp-1">{g.title}</h4>
                                <p className="text-[10px] text-slate-500 line-clamp-2 leading-relaxed">{g.content}</p>
                            </div>
                        ))}
                        ...
                        {filteredKB.length === 0 && (
                            <div className="text-center py-10 text-slate-400 border border-dashed border-slate-200 rounded-xl">
                                ...
                                <button onClick={() => setIsGuidelineModalOpen(true)} className="text-indigo-600 text-xs font-bold mt-2 hover:underline">Add New</button>
                            </div>
                        )}
                    </>
                )}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
             <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden animate-in fade-in zoom-in-95">
                <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                    ...
                    <button onClick={() => setIsGuidelineModalOpen(false)}><X size={20} className="text-slate-400 hover:text-slate-600"/></button>
                </div>
                <div className="p-6 space-y-4">
                    ...
                    <div>
                        <label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1">Protocol Content</label>
                        <textarea 
