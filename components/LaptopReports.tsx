...
export default function LaptopReports({
  ...
  const reportsRef = collection(db, "laptop_reports");
  useEffect(() => {
  ...
  const handleSaveReport = async () => {
    if (!currentReport.deviceInfo.laptopNo) {
      alert("Laptop No is required");
      return;
    }

    const isNew = !currentReport.id;
    const reportId = isNew ? Date.now().toString() : currentReport.id;
    ...
    const historyEntry: ReportHistory = {
      ...
      action: isNew ? "Report Created" : "Report Updated",
      details: isNew
        ? `New report created for Laptop ${currentReport.deviceInfo.laptopNo}`
        : `Report updated. Progress: ${currentReport.progress}%`,
    };
    ...
    const reportToSave: Report = {
      ...
      createdAt: isNew ? serverTimestamp() : currentReport.createdAt,
      updatedAt: serverTimestamp(),
    };

    // 🔥 SAVE TO FIRESTORE (TOP-LEVEL COLLECTION)
    await setDoc(doc(db, "laptop_reports", reportId), reportToSave);
                      ...
                      <tr key={report.id} className="hover:bg-slate-50">
                        ...
                        <td className="px-6 py-4">
                          {report.actionRequired ? (
                            ...
                            <span className="bg-emerald-100 text-emerald-700 px-2 py-1 rounded-md text-xs font-bold whitespace-nowrap border border-emerald-200">
                              Pass
                            </span>
                          )}
                        </td>
